...
cat /kafka/config/server.properties.template | sed \
  ...
  -e "s|{{KAFKA_ADVERTISED_LISTENERS}}|${KAFKA_ADVERTISED_LISTENERS:-INTERNAL_PLAINTEXT://$IP:9092,EXTERNAL_PLAINTEXT://$MINIKUBE_IP}|g" \
  -e "s|{{KAFKA_LISTENER_SECURITY_PROTOCOL_MAP}}|${KAFKA_LISTENER_SECURITY_PROTOCOL_MAP:-INTERNAL_PLAINTEXT:PLAINTEXT,EXTERNAL_PLAINTEXT:PLAINTEXT}|g" \
  -e "s|{{KAFKA_INTER_BROKER_LISTENER_NAME}}|${KAFKA_INTER_BROKER_LISTENER_NAME:-INTERNAL_PLAINTEXT}|g" \
  -e "s|{{KAFKA_LISTENERS}}|${KAFKA_LISTENERS:-INTERNAL_PLAINTEXT://0.0.0.0:9092,EXTERNAL_PLAINTEXT://0.0.0.0:9093}|g" \
